...
//                  event we want, function to run when this event happens<br>
btn.addEventListener('click', e => { <br>
    //we want event parameter, (e) in function  <br><br>
    e.preventDefault();  <br>
    //when we click, it doesn't flash and go away, it stops there because the form is no longer actually submitting<br><br>
    console.log(e.target); <br>
    // e.target means click karne wala btn ke baare me bta dega like : <br>
    <input class.. type... value... so on ><br><br>
    console.log(e.target.className);  <br>
    // <input> ki class name show kardega<br><br>
    document.querySelector("#my-form").style.background = '#ccc';  <br>
...
Mouseover: jb uspe hover karenge tb change hoga <br>
Mouseout: jb usse out, mtlb bahar jaenge, tb changes honge<br>
